...
public class uniform_search{
    ...
    public uniform_search(double[][] m, double holderianConstantP1,double holderianConstantP2, double epsilonExt, double epsilonInt){
	...
	int N = (int) (Math.log(holderianConstantP1/epsilonExt)/Math.log(2)) +1;

	HashMap<Integer, ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>>> Subx = new HashMap();

	  ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>> tmpList = new ArrayList();
	 tmpList.add(temp); 
	Subx.put(0,tmpList);
	for (int i = 1;i<=N;i++){
	    for (HashMap<Integer,ArrayList<ArrayList<Double>>> l : Subx.get(i-1)){
		    
		    ArrayList<HashMap<Integer,ArrayList<ArrayList<Double>>>> listNewSubdivisions = new ArrayList<>();
		    listNewSubdivisions = SubdiviserPdtCartesien(l,3,2);
		    
		    for ( HashMap<Integer,ArrayList<ArrayList<Double>>> element : listNewSubdivisions){
		    	if (!Subx.keySet().contains(i)){
				ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>> listI = new ArrayList();
				listI.add(element);
				Subx.put(i,listI);	
			}
			else{
			    Subx.get(i).add(element);
			}
		    }
	...
	//System.out.println("subx : " + Subx.get(5).size());
	int N2 = (int) (Math.log(holderianConstantP2/epsilonInt)/Math.log(2)) +1;
	
	HashMap<Integer, ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>>> Suby = new HashMap();

        ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>> tmpListy = new ArrayList();
	 tmpListy.add(tempJ2); 
	Suby.put(0,tmpListy);
	for (int i = 1;i<=N2;i++){
	    for (HashMap<Integer,ArrayList<ArrayList<Double>>> l : Suby.get(i-1)){
		    ArrayList<HashMap<Integer,ArrayList<ArrayList<Double>>>> listNewSubdivisions = new ArrayList<>();
		    listNewSubdivisions = SubdiviserPdtCartesien(l,3,2);
		    
		    for ( HashMap<Integer,ArrayList<ArrayList<Double>>> element : listNewSubdivisions){
		    	if (!Suby.keySet().contains(i)){
				ArrayList<HashMap<Integer, ArrayList<ArrayList<Double>>>> listI = new ArrayList();
				listI.add(element);
				Suby.put(i,listI);	
			}
			else{
			    Suby.get(i).add(element);
			}
		    }
...
double maxi = -1000;	
	for (HashMap<Integer, ArrayList<ArrayList<Double>>> elementX : Subx.get(N)){
	    double mini = 10000;
	    for (HashMap<Integer, ArrayList<ArrayList<Double>>> elementY : Suby.get(N2)){
		double s = fonctionRecompense(getValidProbability(elementX),getValidProbability(elementY),true);	
    	    	if (s<mini)
		    mini=s;
	    }
	    if (mini>maxi)
	...
	System.out.println("value maximin : " + maxi);
    }
